...
function DayChangeBanner({
  ...
  const chartData = closes30.map((v, i) => ({
    ...
    trend2:
      i >= offset2
        ? parseFloat((intercept2 + slope2 * (i - offset2)).toFixed(2))
        : null,
  }));

  const trendUp = slope >= 0;
  const highIdx = closes30.reduce(
    (best, v, i) => (v > closes30[best] ? i : best),
    0,
  );
  const lowIdx = closes30.reduce(
    (best, v, i) => (v < closes30[best] ? i : best),
    0,
  );
  ...
  if (weekStart < dates30.length) {
    weekChunks.push({
      x1: dates30[weekStart],
      x2: dates30[dates30.length - 1],
    });
  }
  ...
  const avg5Move = weeklyMoves.length
    ? weeklyMoves.reduce((s, w) => s + Math.abs(w.dollar), 0) /
      weeklyMoves.length
    : 0;
  const max5Up = weeklyMoves.length
    ? weeklyMoves.reduce((b, w) => (w.dollar > b.dollar ? w : b))
    : { dollar: 0 };
  const max5Down = weeklyMoves.length
    ? weeklyMoves.reduce((b, w) => (w.dollar < b.dollar ? w : b))
    : { dollar: 0 };

  const trend2Up = slope2 >= 0;
  const trendLineColor =
    slope >= 0 ? "rgba(20,184,166,0.45)" : "rgba(239,68,68,0.45)";
  const bg =
    trendUp && trend2Up
            ...
            if (trendUp && trend2Up)
              return (
                <Stack gap={2} align="center">
                  ...
                  >
                    Trending Up — Favor selling puts.
                  </Text>
                  <Text size="xs" c="dimmed" ta="center">
                    For best premium, wait for a down day before opening a new
                    position.
                  </Text>
                </Stack>
              );
            if (!trendUp && !trend2Up)
              return (
                <Stack gap={2} align="center">
                  ...
                  >
                    Trending Down — Favor selling calls.
                  </Text>
                  <Text size="xs" c="dimmed" ta="center">
                    For best premium, sell on a down day when IV is elevated.
                  </Text>
                </Stack>
              );
            return (
              <Stack gap={2} align="center">
                <Text size="xs" fw={700} c="dimmed" ta="center">
                  Composite Trend is Neutral — Calls and puts are both viable.
                </Text>
                <Text size="xs" c="dimmed" ta="center">
                  Try to sell puts on down days and calls on up days for better
                  premium.
                </Text>
              </Stack>
            );
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Line
                  ...
                  activeDot={false}
                  stroke={
                    slope2 >= 0 ? "rgba(20,184,166,0.7)" : "rgba(239,68,68,0.7)"
                  }
                  strokeWidth={1.5}
                  strokeDasharray="3 2"
                  connectNulls={false}
                ...
                <Line
                  ...
                  dot={(props) => {
                    ...
                    const isUp =
                      index === 0 || closes30[index] >= closes30[index - 1];
                    return (
                      <circle
                        key={index}
                        cx={cx ?? 0}
                        cy={cy ?? 0}
                        r={2}
                        fill={isUp ? "rgba(20,184,166,1)" : "rgba(239,68,68,1)"}
                    ...
                    content={(props) => {
                      ...
                      return (
                        <text
                          ...
                          fontWeight={700}
                          fill={
                            isHigh ? "rgba(20,184,166,1)" : "rgba(239,68,68,1)"
                          }
                        >
                          ${value.toFixed(2)}
                        </text>
          ...
          <>
            ...
            <Group grow gap="xs">
              <Stack gap={0} align="center">
                ...
                <Text size="xs" fw={700} style={{ color: "rgba(239,68,68,1)" }}>
                  ${max5Down.dollar.toFixed(2)}
                </Text>
              </Stack>
              <Stack gap={0} align="center">
                <Group gap={4} align="center">
...
function SummaryStat({
  ...
  return (
    <Stack gap={2} style={{ flex: 1, minWidth: 0 }}>
      ...
      <Text size="sm" fw={700} c={valueColor}>
        {value}
      </Text>
    </Stack>
  );
...
function daysUntil(expiry: string): number {
  ...
  return Math.max(
    0,
    Math.round(
      (new Date(expiry + "T00:00:00").getTime() - today.getTime()) / 86400000,
    ),
  );
}
...
function progressPct(
  ...
  return Math.max(
    0,
    Math.min(100, ((averagePrice - currentPerShare) / averagePrice) * 100),
  );
}
...
function PositionAdvice({
  ...
  const mask = createMask(privacyMode);
  const isPut = position.putCall === "PUT";
  const lime = "var(--mantine-color-lime-5)";
  const yellow = "var(--mantine-color-yellow-5)";
  const red = "var(--mantine-color-red-5)";
  const statLabel = { fontSize: 9, letterSpacing: "0.04em" } as const;

  // ── profitability ──────────────────────────────────────────────────────────
  const totalCredit = position.averagePrice * position.shortQty * 100;
  const costToClose = Math.abs(position.marketValue);
  const currentPnl = totalCredit - costToClose;
  const dte = daysUntil(position.expiry);
  const totalDays = position.openedAt
    ? Math.max(
        dte + 1,
        Math.round(
          (new Date(position.expiry + "T00:00:00").getTime() -
            new Date(position.openedAt).getTime()) /
            86400000,
        ),
      )
    : 45;
  const daysHeld = Math.max(0, totalDays - dte);
  const expectedPnl = (totalCredit / totalDays) * Math.max(1, daysHeld);
  const profitColor =
    currentPnl < 0 ? red : currentPnl >= expectedPnl ? lime : yellow;

  // ── fees ───────────────────────────────────────────────────────────────────
  const currentPricePerShare =
    position.shortQty > 0 ? costToClose / (position.shortQty * 100) : 0;
  const noFee = currentPricePerShare <= 0.05;
  const totalFee = noFee ? 0 : position.shortQty * 0.65;
  const closeNowPnl = currentPnl - totalFee;
  const closeNowColor =
    closeNowPnl < 0 ? red : closeNowPnl >= expectedPnl ? lime : yellow;
  const feeColor = noFee ? lime : red;
  const holdColor = lime;

  return (
    <Group gap="xl" justify="center" align="center">
      <Stack gap={0} align="center">
        <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={statLabel}>
          Profit vs Decay
        </Text>
        <Text size="xs" style={{ color: profitColor }}>
          {mask(`$${currentPnl.toFixed(2)}`)} /{" "}
          {mask(`$${expectedPnl.toFixed(2)}`)}
        </Text>
      </Stack>
      <Stack gap={0} align="center">
        <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={statLabel}>
        ...
        <Text size="xs" style={{ color: feeColor }}>
          {noFee ? "$0.00" : `$${totalFee.toFixed(2)}`}
        </Text>
      </Stack>
      <Stack gap={0} align="center">
        <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={statLabel}>
        ...
        <Text size="xs" style={{ color: closeNowColor }}>
          {mask(`$${closeNowPnl.toFixed(2)}`)}
        </Text>
      </Stack>
      <Stack gap={0} align="center">
        <Text size="xs" c="dimmed" tt="uppercase" fw={600} style={statLabel}>
        ...
        <Text size="xs" style={{ color: holdColor }}>
          {mask(`$${totalCredit.toFixed(2)}`)}
        </Text>
      </Stack>
    </Group>
  );
...
function buildPutRows(
  ...
  const safeEdge = putStrikeForLevel(
    levels[Math.min(minSafe, levels.length - 1)],
  );
  ...
  const itmCeiling =
    highestPositionStrike !== null &&
    highestPositionStrike > itmNearStrike + 0.5
...
const POSITION_BORDER = "1px solid var(--mantine-color-dark-3)";
...
function PositionCells({
  ...
  const expiryLabel =
    position.expiry?.length >= 10
      ? new Date(position.expiry.slice(0, 10) + "T00:00:00").toLocaleDateString(
          "en-US",
  ...
  const elapsedPct = Math.min(
    100,
    Math.max(0, ((totalDays - dte) / totalDays) * 100),
  );
  const dteColor =
    elapsedPct <= 50
  ...
  return (
    <>
      <Table.Td
        ta="center"
        style={withBorder ? { borderTop: POSITION_BORDER } : undefined}
      >
          ...
          return (
            <Stack gap={1} align="flex-end">
              <Group gap={3} align="center" wrap="nowrap">
                ...
                />
                <Text style={{ fontSize: 10 }} c="dimmed">
                  {currentPricePerShare.toFixed(2)}
                </Text>
              </Group>
      ...
      <Table.Td
        style={{
          minWidth: 70,
          ...(withBorder
            ? { borderTop: POSITION_BORDER, borderRight: POSITION_BORDER }
            : {}),
        }}
      >
        <Stack gap={2}>
          <Group justify="space-between" wrap="nowrap">
            ...
            />
            <Text size="xs" style={{ color: dteColor, whiteSpace: "nowrap" }}>
              {dte}d
            </Text>
          </Group>
          <Box
          ...
          <Box style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <Text size="xs" c="dimmed">
              {mask(
                `$${(position.averagePrice * position.shortQty * 100).toFixed(2)}`,
              )}
            </Text>
          </Box>
        </Stack>
    ...
    <Stack gap="xs">
      ...
      {btoPositions.length > 0 && (
        ...
        >
          ...
          <Text size="xs" c="dimmed">
            {btoPositions
              .map((p) => `$${p.strike.toFixed(2)} put ×${p.longQty}`)
              .join(", ")}{" "}
            — sell to close ASAP
          </Text>
        </Alert>
...
function OptionsPageInner() {
  ...
  const { riskStrikeCall, riskStrikeAvgCall, riskStrikePut, riskStrikeAvgPut } =
    useMemo(() => {
      ...
      for (let i = 1; i < dates30.length; i++) {
        if (daysOfWeek30[i] === 1) {
          weekChunks.push({ x1: dates30[ws], x2: dates30[i - 1] });
          ws = i;
        }
      }
      if (ws < dates30.length)
        weekChunks.push({ x1: dates30[ws], x2: dates30[dates30.length - 1] });
      const moves = weekChunks.map(
        (c) =>
  ...
  if (isMobile) {
    return (
      <Stack gap="md">
        ...
        <DayChangeBanner
          closes30={quote.closes30}
          dates30={quote.dates30}
