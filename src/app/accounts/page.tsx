...
import {
  Text, Stack, Group, Paper, Box, Skeleton, ScrollArea, Switch,
} from "@mantine/core";
import { Outfit } from "next/font/google";
import { useApp } from "@/lib/context/AppContext";
import type { Account } from "@/lib/context/AppContext";
import type { FilledOrder, OptionPosition, WorkingOrder } from "@/lib/schwab/parse";
...
function buildLevelsAndCurrentLevel(
  ...
  if (!s.startingCash || !s.initialLotPrice || !s.sellPercentage || !s.reductionFactor) {
    return { levels: [], currentLevel: null };
  }
  const levels = computeLevels(s.startingCash, s.initialLotPrice, s.sellPercentage, s.reductionFactor);
  if (levels.every((l) => l.shares === 0)) return { levels: [], currentLevel: null };

  const resetDate = s.levelResetDate ?? null;
  const orders = filledOrders.filter((o) => o.accountNumber === account.accountNumber);
  const relevant = resetDate ? orders.filter((o) => new Date(o.time) >= resetDate) : orders;
...
function CardLabel({ children }: { children: React.ReactNode }) {
  return (
    <Text c="dimmed" tt="uppercase" fw={600} ta="center" w="100%" style={CARD_LABEL_STYLE}>
      {children}
    </Text>
  );
}
...
function AccountValueWidget({
  balance, pendingBuyCost, cspCollateral, color, mask,
}: {
  balance: AccountBalance | null;
  pendingBuyCost: number | null;
  cspCollateral: number | null;
    ...
    <Paper p="md" radius={CARD_RADIUS} style={{ background: bg }}>
      <Stack gap="md">
        ...
        <AnimatedNumber
          ...
          style={{ fontSize: "2.25rem", fontWeight: 700, lineHeight: 1, color: "white" }}
        />
        <Box
          style={{
        ...
        >
          {total > 0 && segments.map((s) => {
            ...
            return (
              <Box key={s.key} style={{ width: `${pct}%`, height: "100%", background: s.color, flexShrink: 0 }} />
            );
          })}
        </Box>
        <Stack gap={4}>
            ...
            return (
              <Group key={s.key} justify="space-between" wrap="nowrap">
                ...
                <Group gap={6} wrap="nowrap">
                  ...
                  <Text size="xs" c="dimmed" w={36} ta="right">{fmt(pct, 1)}%</Text>
                </Group>
              </Group>
            );
          })}
        ...
        <Stack gap={4} mt="auto">
          ...
          {cspCollateral != null && (
            <Group justify="space-between">
              ...
              <Text size="xs" fw={600} c="grape">{mask(`$${fmt(cspCollateral, 0)}`)}</Text>
            </Group>
          )}
          <Group justify="space-between">
    ...
    return (
      <Paper p="md" radius={CARD_RADIUS} style={{ background: bg }}>
        <Stack align="center" gap={8}>
          ...
          <Text size="sm" c="dimmed">{currentLevel == null ? "Not configured" : "—"}</Text>
        </Stack>
      </Paper>
    );
  }
      ...
      <Stack align="center" gap={12}>
        ...
        <Group gap={0} wrap="nowrap" justify="center">
          {Array.from({ length: end - start + 1 }, (_, i) => {
            ...
            const gapRight = idx < end ? Math.max(0, Math.round(5 * (1 - t * t))) : 0;
            return (
              <Box
                key={idx}
                style={{
                  ...
                  flexShrink: 0, marginRight: gapRight, opacity,
              ...
              >
                <Text fw={isCurrent ? 700 : 400} c={isCurrent ? "white" : "dimmed"} lh={1} style={{ fontSize }}>
                  {idx}
                </Text>
              </Box>
            );
...
function MiniStatCard({
  ...
  const bg = useCardBg(color);
  return (
    <Paper p="sm" radius={CARD_RADIUS} style={{ background: bg, flex: 1, minWidth: 0 }}>
      <Stack gap={6} align="center">
        <Text c="dimmed" tt="uppercase" fw={600} ta="center" style={CARD_LABEL_STYLE}>{label}</Text>
        <Text
          className={outfit.className}
          style={{ fontSize: "1.6rem", fontWeight: 700, lineHeight: 1, color: "white" }}
        >
          {String(value)}
        </Text>
      </Stack>
...
function GainLossWidget({
  gain, gainPct, annualROI, color, mask,
}: {
  gain: number | null;
  gainPct: number | null;
  annualROI: number | null;
  ...
  const bg = useCardBg(color);
  const gainColor = gain == null ? "white" : gain >= 0 ? "white" : "var(--mantine-color-red-6)";
  ...
  return (
    <Paper p="md" radius={CARD_RADIUS} style={{ background: bg }}>
      <Stack gap="md" align="center">
        ...
        <AnimatedNumber
          value={gain == null ? "—" : mask(`${gain >= 0 ? "" : "-"}$${fmt(Math.abs(gain), 0)}`)}
          className={outfit.className}
          style={{ fontSize: "2rem", fontWeight: 700, lineHeight: 1, color: gainColor }}
        />
        {(gainPct != null || annualROI != null) && (
          <Text size="sm" c="dimmed" ta="center">
            ...
            {annualROI != null && (
              <Text span fw={500}>
                {annualROI >= 0 ? "+" : ""}{annualROI.toFixed(1)}%/yr
              </Text>
            )}
          </Text>
        )}
...
function AccountColumn({
  ...
  const { account, balance, levels, currentLevel, tqqqShares, openOptions, tradesToday,
    pendingBuyCost, cspCollateral, gain, gainPct, annualROI } = data;
  ...
  return (
    <Stack gap="md" style={{ width: COLUMN_WIDTH, flexShrink: 0 }}>
      ...
      <Group gap={8} justify="center" wrap="nowrap">
        ...
        />
        <Text fw={700} size="sm" style={{ whiteSpace: "nowrap", opacity: enabled ? 1 : 0.4 }}>
          {account.accountName}
        </Text>
      </Group>
      ...
      <Box style={{
        ...
        <Stack gap="md">
          ...
          <Group gap="xs" wrap="nowrap">
            ...
            <MiniStatCard label="Shares" value={tqqqShares.toLocaleString()} color={color} />
          </Group>
          <GainLossWidget gain={gain} gainPct={gainPct} annualROI={annualROI} color={color} mask={mask} />
...
function CombinedColumn({
  accounts, balances, tqqqShares, openOptions, tradesToday,
  pendingBuyCost, cspCollateral, gain, gainPct, avgLevel, anyLevels, mask,
}: {
  ...
  const combinedBalance: AccountBalance | null = balances.length > 0 ? {
    ...
    moneyMarketValue:       balances.reduce((s, b) => s + b.moneyMarketValue, 0),
    ...
    cashAvailableForTrading: balances.reduce((s, b) => s + b.cashAvailableForTrading, 0),
  } : null;

  return (
    <Stack gap="md" style={{ width: COLUMN_WIDTH, flexShrink: 0 }}>
      <Group gap={8} justify="center">
        <Text fw={700} size="sm" c="dimmed">Combined</Text>
      </Group>
      ...
      <Group gap="xs" wrap="nowrap">
        ...
        <MiniStatCard label="Shares" value={tqqqShares.toLocaleString()} color="gray" />
      </Group>

      <GainLossWidget gain={gain} gainPct={gainPct} annualROI={null} color="gray" mask={mask} />
...
function AccountsPageInner() {
  const {
    accounts, balances, allFilledOrders, allWorkingOrders, allOptionPositions,
    allTqqqShares, privacyMode, snapshotLoading,
  } = useApp();
  ...
  const accountData = useMemo<AccountData[]>(() => accounts.map((account) => {
    const balance = balances.find((b) => b.accountNumber === account.accountNumber) ?? null;
    const { levels, currentLevel } = buildLevelsAndCurrentLevel(account, allFilledOrders);

    // Owned level indices for pending cost calculation
    const resetDate = account.settings.levelResetDate ?? null;
    const orders = allFilledOrders.filter((o) => o.accountNumber === account.accountNumber);
    const relevant = resetDate ? orders.filter((o) => new Date(o.time) >= resetDate) : orders;
    const lastFillSide = new Map<number, "BUY" | "SELL">();
    for (const o of relevant) {
      const idx = matchLevel(levels, o.shares, o.fillPrice);
    ...
    const ownedIndices = new Set(
      [...lastFillSide.entries()].filter(([, s]) => s === "BUY").map(([i]) => i),
    );

    const tqqqShares = allTqqqShares[account.accountNumber] ?? 0;
    const positions = allOptionPositions.filter((o) => o.accountNumber === account.accountNumber);
    const openOptions = positions.length;
    const tradesToday = allFilledOrders.filter(
      (o) => o.accountNumber === account.accountNumber && toDateKey(new Date(o.time)) === today,
    ).length;
    const pendingBuyCost = buildPendingBuyCost(account, allWorkingOrders, levels, ownedIndices);
    const cspCollateral = (() => {
      const puts = positions.filter((p) => p.putCall === "PUT" && p.shortQty > 0);
      return puts.length > 0 ? puts.reduce((s, p) => s + p.strike * 100 * p.shortQty, 0) : null;
    })();

    const startingCash = account.settings.startingCash;
    const startingDate = account.settings.startingDate;
    const totalValue = balance?.totalValue ?? null;
    const gain = startingCash != null && totalValue != null ? totalValue - startingCash : null;
    const gainPct = gain != null && startingCash ? (gain / startingCash) * 100 : null;
    const daysIn = startingDate ? Math.max(1, (Date.now() - startingDate.getTime()) / 86400000) : null;
    const annualROI = gainPct != null && daysIn != null ? (gainPct / daysIn) * 365 : null;

    return {
      account, balance, levels, currentLevel, tqqqShares, openOptions,
      tradesToday, pendingBuyCost, cspCollateral, gain, gainPct, annualROI,
    };
  }), [accounts, balances, allFilledOrders, allWorkingOrders, allOptionPositions, allTqqqShares, today]);
  ...
  const combined = useMemo(() => {
    const enabled = accountData.filter((d) => !disabledAccounts.has(d.account.accountNumber));
    const allHaveGain = enabled.length > 0 && enabled.every((d) => d.gain != null);
    const totalGain = allHaveGain ? enabled.reduce((s, d) => s + (d.gain ?? 0), 0) : null;
    const totalStartingCash = enabled.reduce((s, d) => s + (d.account.settings.startingCash ?? 0), 0);
    const configuredLevels = enabled.filter((d) => d.currentLevel != null && d.currentLevel >= 0);
    const avgLevel = configuredLevels.length > 0
      ? Math.round(configuredLevels.reduce((s, d) => s + (d.currentLevel ?? 0), 0) / configuredLevels.length)
      : null;
    ...
    return {
      ...
      gain: totalGain,
      gainPct: totalGain != null && totalStartingCash > 0 ? (totalGain / totalStartingCash) * 100 : null,
      avgLevel,
      anyLevels,
    };
  ...
  if (snapshotLoading) {
    return (
      <Stack gap="md">
        <Text fw={700} size="xl" ta="center">All Accounts</Text>
        <Group gap="md" align="flex-start" wrap="nowrap">
          {[0, 1, 2].map((i) => (
            <Stack key={i} gap="md" style={{ width: COLUMN_WIDTH, flexShrink: 0 }}>
  ...
  return (
    <Stack gap="md">
      ...
      <ScrollArea type="auto" offsetScrollbars>
        <Group gap="md" align="flex-start" justify="center" wrap="nowrap" pb={8}>
