...
import { Modal, Stack, Text, Group, ColorSwatch, NumberInput, Divider, SimpleGrid } from "@mantine/core";
...
export default function SettingsModal({ opened, onClose }: SettingsModalProps) {
  ...
  if (!activeAccount) {
    return (
      <Modal opened={opened} onClose={onClose} title="Account Settings" size="md" padding="lg">
            ...
            {COLORS.map((color) => (
              ...
              >
                {activeAccount.color === color && (
                  <IconCheck size={isMobile ? 10 : 13} color="white" stroke={3} />
                )}
              </ColorSwatch>
            ))}
          </Group>
